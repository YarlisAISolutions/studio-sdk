...
describe('MyBotBoxClient', () => {
  let client: MyBotBoxClient

  beforeEach(() => {
    client = new MyBotBoxClient({
      apiKey: 'test-api-key',
      baseUrl: 'https://test.mybotbox.com',
    })
    vi.clearAllMocks()
  })
  ...
  describe('constructor', () => {
    it('should create a client with correct configuration', () => {
      expect(client).toBeInstanceOf(MyBotBoxClient)
    })

    it('should use default base URL when not provided', () => {
      const defaultClient = new MyBotBoxClient({
        apiKey: 'test-api-key',
      })
      expect(defaultClient).toBeInstanceOf(MyBotBoxClient)
    })
  })
  ...
  describe('getJobStatus', () => {
    ...
    it('should handle job not found error', async () => {
      ...
      await expect(client.getJobStatus('invalid-task')).rejects.toThrow(MyBotBoxError)
  ...
  describe('getUsageLimits', () => {
    it('should fetch usage limits with correct structure', async () => {
      ...
      // Verify correct endpoint was called
      const calls = vi.mocked(fetch.default).mock.calls
      expect(calls[0][0]).toBe('https://test.mybotbox.com/api/users/me/usage-limits')
    })
    ...
    it('should handle unauthorized error', async () => {
      ...
      await expect(client.getUsageLimits()).rejects.toThrow(MyBotBoxError)
...
describe('MyBotBoxError', () => {
  it('should create error with message', () => {
    const error = new MyBotBoxError('Test error')
    expect(error.message).toBe('Test error')
    expect(error.name).toBe('MyBotBoxError')
  })

  it('should create error with code and status', () => {
    const error = new MyBotBoxError('Test error', 'TEST_CODE', 400)
    expect(error.message).toBe('Test error')
